...
public class Lucky implements Task {
    ...
    public long getLuckyCount(int n) {
        ...
        // Если x - это количество возможных комбинаций, в которых сумма первых n цифр равна конкретному значению, то столько же возможных комбинаций будет для n последних цифр.
        // Чтобы получить итоговое количество комбинаций, где сумма первых n цифр равна сумме n последних цифр и равна конкретному значению, нужно x умножить на x.
        long count = 0;
        for (long sum : sums) {
            count += sum * sum;
        }
